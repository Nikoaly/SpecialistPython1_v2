...
t = int(input("Введите время его движения: "))

distance = v * t #вычисляем расстояние, которое проедет байкер
if v >= 0:
    stop_point = distance % 109 #вычисляем номер отметки (движения по часовой стрелке)
else:
    stop_point = (109 - distance % 109) % 109 #вычисляем номер отметки (движения против часовой стрелки)

print("Байкер остановится на отметке:", stop_point)
```
